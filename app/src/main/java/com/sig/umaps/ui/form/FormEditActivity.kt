...
import com.sig.umaps.databinding.ActivityFormEditBinding
...
class FormEditActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val birthDate = sharedPref.getString(PREFS_BIRTH_DATE, null)
        ...
        setCalendar()
        enabledButton()
        setData(image, name, email, birthDate, userStatus, accessToken, userId)
    }
    ...
    private fun setData(
        ...
        binding.apply {
            Glide.with(this@FormEditActivity)
                .load(image)
                .into(imgProfile)
            edtName.setText(name)
            edtEmail.setText(email)
            edtDate.setText(birthDate)
            edtStatus.setText(status)

            btnRegister.setOnClickListener {
                formViewModel.apply {
                    val setBirthDate = edtDate.text.toString()
                    val setUserStatus = edtStatus.text.toString()
                    val editor = sharedPref.edit()
                    editor.putString(PREFS_BIRTH_DATE, setBirthDate)
                    editor.putString(PREFS_USER_STATUS, setUserStatus)
                    editor.apply()
                    putDataUser(accessToken, userId, image, setBirthDate, setUserStatus)
                    isLoading.observe(this@FormEditActivity, { isLoading ->
                        showLoading(isLoading)
                    })

                    Intent(this@FormEditActivity, NavigationContainerActivity::class.java).apply {
                        startActivity(this)
                    }
                }
            }
